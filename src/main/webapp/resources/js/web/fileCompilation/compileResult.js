...
function initMethod(){
	...
	$("button.export").on("click",function(){
		...
		if(files.length==0){
			layer.alert("请选择编研目录导出！", {skin: 'layui-layer-lan',closeBtn: 0});
			return;
		}
		if(files.length>20){
			layer.alert("选择的编研目录不能超过20条！", {skin: 'layui-layer-lan',closeBtn: 0});
			return;
		}
		...
		ids = ids.join(',');
		location.href = Util.getRootPath() + "/w/compilation/exportCompilationZip"+'/'+ids;
