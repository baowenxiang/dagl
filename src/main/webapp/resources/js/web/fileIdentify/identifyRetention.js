...
function initMethod(){
	...
	//点击修改保管期限按钮
	$('button.modifyretention').on("click",function(){
		...
		var options = {}
		options.url = Util.getRootPath() + "/w/fileidentify/identifyInfo/toModifyRetentionView?tablename="+tablename+"&ids="+businessStr+"&flag=NEW";
...
function initGrid(tablename){
	...
	grid=$.fn.DtGrid.init({
		loadURL : Util.getRootPath() + "/w/fileidentify/identifyInfo/getIdentifyFiles",
	    ajaxLoad : true,
	    ...
	    columns : [
					{
		       		...
		       		},
		       		{
						...
						resolution:function(value, record, column, grid, dataNo, columnNo){
							if(record.isarchive == '6'){
								return "审批中";
							}else if(record.isarchive == '7'){
								return "已拒绝";
							}else{
								return "未修改";
							}
