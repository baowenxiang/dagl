	    	...
	    	$.ajax({
	             ...
	             success: function(result) {
	            	 ...
	            	 for(var i = 0;i<headers.length;i++){
	            		 ...
	            		 if(headers[i].id=="tm" || headers[i].id=="ajtm"){
							 headers[i].headerStyle = "width:30%";
							 headers[i].columnClass = "text-left";
						 }
	            		 
						 if(headers[i].id=="dh"){
							 headers[i].headerStyle = "width:14%";
							 
						 }
						 
						 if(headers[i].id=="zrz"){
							 headers[i].headerStyle = "width:19%";
							 headers[i].columnClass = "text-left";
						 }
        								...
        								if(result.success){
        									...
        									if(datas.length>0){
            									for(var i = 0;i<datas.length;i++){
            										...
            										var wjm = datas[i].wjm;
            										if(wjlx == 'pdf'){
            											html += '<img dagl-uuid="' + datas[i].id  + '" title="'+wjm+'" fileType="'+wjlx+'"  src="' + Util.getRootPath() + '/resources/img/icon_16/pdf.svg" />';
            										}else if(wjlx == 'doc' || wjlx == 'docx'){
            											html += '<img dagl-uuid="' + datas[i].id  + '" title="'+wjm+'" fileType="'+wjlx+'" src="' + Util.getRootPath() + '/resources/img/icon_16/word.svg" />';
            										}else if(wjlx == 'xlsx' || wjlx == 'xls'){
            											html += '<img dagl-uuid="' + datas[i].id  + '" title="'+wjm+'" fileType="'+wjlx+'"  src="' + Util.getRootPath() + '/resources/img/icon_16/excel.svg" />';
            										}else if(Util.isAudio(wjlx)){
            											html += '<img dagl-uuid="' + datas[i].id  + '" title="'+wjm+'" fileType="'+wjlx+'" src="' + Util.getRootPath() + '/resources/img/icon_16/音频.svg" />';
            										}else if(Util.isVedio(wjlx)){
            											html += '<img dagl-uuid="' + datas[i].id  + '" title="'+wjm+'" fileType="'+wjlx+'" src="' + Util.getRootPath() + '/resources/img/icon_16/视频.svg" />';
            										}else if(Util.isPIC(wjlx)){
               											html += '<img dagl-uuid="' + datas[i].id  + '" title="'+wjm+'" fileType="'+wjlx+'" src="' + Util.getRootPath() + '/resources/img/icon_16/pic.svg" />';
            										}else{
            											html += '<img dagl-uuid="' + datas[i].id  + '" title="'+wjm+'" fileType="'+wjlx+'" src="' + Util.getRootPath() + '/resources/img/icon_16/文件.svg" />';
            										}
            									}
        									}
        								}
...
function initMethod(){
			...
			success:function(data,textStatus,jqXHR){
				if(data.success){
					if(data.datas.length>0){
						isProcessState = true;
						layer.alert("文档审批中不能删除！", {  skin: 'layui-layer-lan',closeBtn: 0});
						return;
					}
				}
			},
    ...
    $("#tableDataCollect").on("click", "img", function(){
    	...
    	var fileType = $.trim($(this).attr("fileType"));
    		$.ajax({
    			type: "GET",
