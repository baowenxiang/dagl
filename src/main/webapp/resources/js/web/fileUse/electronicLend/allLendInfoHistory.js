...
function initTable(){
	grid=$.fn.DtGrid.init({
		loadURL : Util.getRootPath() + "/w/fileuse/electronicLendHistory/getAllElectronicLendList",
	    ajaxLoad : true,
...
function initMethod(){
    ...
    $("#tableElectronicLend").on("click","a.delete",function(){
		...
		$.ajax({
			url : Util.getRootPath()+"/w/fileuse/electronicLendHistory/delete",
			type:'POST', //GET
			async:true,
			data:{
				id:id
    ...
    $("button.export").on("click",function(){
    	...
    	if (records.length==0) {
    		//打包导出
			location.href = Util.getRootPath() + "/w/fileuse/electronicLendHistory/exportElectronicLend/null";
		}else{
			var ids = [];
			...
			ids = ids.join(',');
			//打包导出
			location.href = Util.getRootPath() + "/w/fileuse/electronicLendHistory/exportElectronicLend/"+ids;
		}
    })
