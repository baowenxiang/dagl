...
public class EEPUtil {
	...
	/**
	 * @Method: createElecRecdsEncaPkg 
	 * @Description: TODO
	 * @param signedobj 
	 * @param elecsignatureBlock
	 * @param lockSignature
	 * @return
	 */
	public ElecRecdsEncaPkg createElecRecdsEncaPkg(SignedObj signedobj, ElecsignatureBlock elecsignatureBlock, LockSignature lockSignature){
        ElecRecdsEncaPkg elecRecdsEncaPkg = factory.createElecRecdsEncaPkg();
        // 封装包格式描述
        elecRecdsEncaPkg.setEncaPkgFormatDesc(this.pkgdesc);
        // 版本
        ...
        elecRecdsEncaPkg.setSignedObj(signedobj);
        // 电子签名块
        elecRecdsEncaPkg.setElecsignatureBlock(elecsignatureBlock);
        // 锁定签名
        elecRecdsEncaPkg.setLockSignature(lockSignature);
        return elecRecdsEncaPkg;
    }
	...
	public Encode createEncode(ElecAttribute elec, DigitalAttribute attr, String desc, String renderKey, EncodeData data){
		...
		// 编码描述
		encode.setRenderKey(renderKey);
		// 反编码关键字
		encode.setEocodeDesc(desc);
		// 编码数据
	...
	public ElecSignature createElecSignature(String signid, String rule, byte[] result, List<CertiBlock> certs, String algo){
	   return this.createElecSignature(signid, rule, null, null, result, certs, algo); 
	}

	
	/**
	 * @Method: createElecSignature 
	 * @Description: TODO
	 * @param signid
	 * @param rule
	 * @param time
	 ...
	 * @return
	 */
	public ElecSignature createElecSignature(String signid, String rule, XMLGregorianCalendar time, String people, byte[] result, List<CertiBlock> certs, String algo){
	    ElecSignature signature = factory.createElecSignature();
	    // 签名标识符
	    signature.setSignatureId(signid);
	    // 签名规则
	    signature.setSignatureRole(rule);
	    // 签名时间
	    if(time != null) signature.setSignatureTime(time);
	    // 签名人
	    if(time != null) signature.setSignaturePeople(people);
	    // 签名结果
	    signature.setSignatureResult(result);
	    // 证书
	    if(certs != null){
	        for(CertiBlock cert: certs){
	            signature.getCertiBlock().add(cert);
	        }
	    }
	    // 签名算法标识
	    signature.setSignatureAlgoId(algo);
	    return signature;
	}
	
	public LockSignature createLockSignature(ElecSignature signid, String rule, byte[] result, List<CertiBlock> certs, String algo){
	   return this.createLockSignature(signid, rule, null, null, result, certs, algo); 
	}
	
	public LockSignature createLockSignature(ElecSignature signid, String rule, XMLGregorianCalendar time, String people, byte[] result, List<CertiBlock> certs, String algo){
	    LockSignature lockSignature = factory.createLockSignature();
	    // 被锁定签名标识符
	    lockSignature.setLockedSignatureId(signid);
	    // 签名规则
	    lockSignature.setSignatureRole(rule);
	    // 签名时间
	    if(time != null) lockSignature.setSignatureTime(time);
	    // 签名人
	    if(people != null) lockSignature.setSignaturePeople(people);
	    // 签名结果
	    lockSignature.setSignatureResult(result);
	    if(certs != null){
	        // 证书块
	        for(CertiBlock cert : certs){
	            lockSignature.getCertiBlock().add(cert);
	        }
	    }
	    // 签名算法标识
	    lockSignature.setSignatureAlgoId(algo);
	    return lockSignature;
	}
	
	/**
	 * @Method: createCertiBlock 
	 * @Description: TODO
	 * @param certs
	 * @return
	 */
	public CertiBlock createCertiBlock(List<byte[]> certs){
	    return this.createCertiBlock(certs, null);
	}
	/**
	 * @Method: createCertiBlock 
	 * @Description: TODO
	 ...
	 * @return
	 */
	public CertiBlock createCertiBlock(List<byte[]> certs, String cretref){
	    CertiBlock certiBlock = factory.createCertiBlock();
	    for(byte[] cert: certs){
	        certiBlock.getCertificate().add(cert);
	    }
	    if(cretref != null) certiBlock.setCertificateRef("");
	    return certiBlock;
	}
	
	public ElecsignatureBlock createElecsignatureBlock(List<ElecSignature> elecsignatures){
	    ElecsignatureBlock elecsignatureBlock = factory.createElecsignatureBlock();
	    for(ElecSignature elecSignature: elecsignatures){
	        elecsignatureBlock.getElecSignature().add(elecSignature);
	    }
