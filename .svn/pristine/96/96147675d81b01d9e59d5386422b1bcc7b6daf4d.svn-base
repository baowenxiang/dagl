...
function initMethod(){
	...
	$("#tableDataCollect").on("click","a.delete",function(){
		...
		$.ajax({
			url : Util.getRootPath()+"/w/zlzl/oazlsj/deleteZlsjById",
			type:'POST', //GET
			async:true,
			data:{
				id:id
	...
	/**$("#tableDataCollect").on("click",".update",function(){
    	...
    	var options = {}
    	var url = Util.getRootPath() + "/w/zlzl/oazlsj/detail/"+tablename+"/"+dataId;
    	options.url = url;
    	options.title = "查看记录";
...
function initTable(){
	grid=$.fn.DtGrid.init({
		loadURL : Util.getRootPath() + "/w/zlzl/oazlsj/getZlsjList1",
	    ajaxLoad : true,
	    ...
	    columns : [
					{
		       		...
		       		},
		       		{
		       			...
		       			resolution:function(value, record, column, grid, dataNo, columnNo){
		       				if(record.delFlag == '0'){
		       					return '<span title="'+value+'" style="cursor: pointer;">未归档</span>';
		       				}else{
		       					return '<span title="'+value+'" style="cursor: pointer;">已归档</span>';
		       				};
						}
		       		},
		       		{
		       			...
		       			resolution:function(value, record, column, grid, dataNo, columnNo){
           				 ...
           				 var html = "";
           				 Util.ajaxJsonSync(
       						 Util.getRootPath()+"/w/zlzl/oazlsj/Oafj",
       	                        {
       							 id : value
       	                        },
       	                        function(result){
