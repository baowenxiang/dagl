...
function initGrid(){
	...
	grid = $.fn.DtGrid.init({
		loadURL : Util.getRootPath() + "/w/fileidentify/identifyContent/getToDoTasks?processId="+processId,
	    ajaxLoad : true,
...
function initMethod(){
	$("#tableDiv").on("click",".toLeader",function(){
		...
		var options = {}
		options.url = Util.getRootPath() + "/w/fileidentify/identifyContent/nextstepView?tablename="+tablename+"&ids="+ids+"&flag=HANDLE";;
