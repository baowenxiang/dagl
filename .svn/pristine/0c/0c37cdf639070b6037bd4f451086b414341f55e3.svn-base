			...
			var dtGridObject = {
				...
				searchLoad: function(){
					...
					if(pass){
						...
						dtGridReflectionObj.advanceQueryParameter.advanceQueryConditions = [];
				...
				constructGrid : function(){
					...
					if(dtGridReflectionObj.option.showHeader!=false){
						...
						if(dtGridReflectionObj.option.check){
							gridContent += '	<th class="check-column" width="3%"><input type="checkbox" id="dt_grid_'+dtGridReflectionObj.option.id+'_check" value="check"></th>';
						}
					...
					$('#dt_grid_'+dtGridReflectionObj.option.id).colResizable({
		                liveDrag:true, 
				...
				bindCheckEvent : function(func, checkInput, e){
					...
					//获取扩展行jQuery对象
					var extraCell = $('#dt_grid_'+gridId+' .dt-grid-extra-columns>td[dataNo="'+dataNo+'"]');
					func(checkInput.checked, record, dtGridReflectionObj, dataNo, row, extraCell, checkInput, e);
				},
				//构建分页内容
				...
				advanceSort : function(){
					...
					var dtGridReflectionObj = this;
					//如果已经初始化，则调用显示
					if(dtGridReflectionObj.init.advanceSortWindowIsInit){
						$('#advanceSort_'+dtGridReflectionObj.option.id+'Modal').modal('show');
						return;
					}
					//加载查询内容
					var content = '';
					content += $.fn.DtGrid.tools.getWindowStart('advanceSort_'+dtGridReflectionObj.option.id, '<i class="fa fa-sort"></i>&nbsp;&nbsp;排序');
					content += '<div class="modal-body advance-query">';
					content += '	<ul class="nav nav-tabs" role="tablist">';
					content += '	</ul>';
					content += '	<div class="tab-content">';
					content += '		<div class="tab-pane active" id="advance_query_sort_'+dtGridReflectionObj.option.id+'">';
					...
					content += '</div>';
					var buttons = '';
					buttons += '<button type="button" class="btn btn-primary" id="doAdvanceSort_'+dtGridReflectionObj.option.id+'">';
...
//					
					...
					//绑定方法：执行高级查询
					$('#doAdvanceSort_'+dtGridReflectionObj.option.id).click(function(){
						dtGridReflectionObj.doAdvanceQuery();
					});
					//显示排序
					$('#advanceSort_'+dtGridReflectionObj.option.id+'Modal').modal('show');
					//标识初始化完成
					dtGridReflectionObj.init.advanceSortWindowIsInit = true;
				},
				/**
				 * 高级查询相关
				 */
...
//					});
				...
				doAdvanceQuery : function(){
					...
					if(pass){
						...
						$('#advanceSort_'+dtGridReflectionObj.option.id+'Modal').modal('hide');
					}
				},
				//高级查询执行
				getAdvanceQueryConditions : function(){
					//定义表格对象映像
					var dtGridReflectionObj = this;
					//校验
					var pass = true;
				...
				getAdvanceSortConditions:function(){
					//定义表格对象映像
					var dtGridReflectionObj = this;
					//校验
					var pass = true;
				...
				insertConditionTr : function(){
					...
					content += '			<option value=""></option>';
					content += '			<option value="0">等于</option>';
					content += '			<option value="1">不等于</option>';
					content += '			<option value="2">包含</option>';
					content += '			<option value="3">开头为</option>';
					content += '			<option value="4">结尾为</option>';
					content += '			<option value="5">大于</option>';
					content += '			<option value="6">大于等于</option>';
					content += '			<option value="7">小于</option>';
					content += '			<option value="8">小于等于</option>';
					content += '			<option value="9">为空</option>';
					content += '			<option value="10">不为空</option>';
					content += '		</select>';
					content += '	</td>';
					...
					content += '			<option value=""></option>';
					content += '			<option value="0">并且</option>';
					content += '			<option value="1">或者</option>';
					content += '		</select>';
					content += '	</td>';
					content += '</tr>';
		...
		tools : {
			...
			getWindowStart : function(id, title){
				...
				content += '	<div style="width:81%;"  class="modal-dialog">';
			...
			openToast : function(content, level, time){
				...
				var toastContent = '<div class="toast '+levelClass+'" style="top:' + top +'px;bottom:' + top + 'px;z-index:'+$.fn.DtGrid.tools.toastZIndex+'">'+content+'</div>';
