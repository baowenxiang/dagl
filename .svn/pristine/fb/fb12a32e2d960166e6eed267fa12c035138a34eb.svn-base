...
function initGrid(){
	...
	grid = $.fn.DtGrid.init({
		loadURL : Util.getRootPath() + "/w/fileidentify/identifyInfo/getToDoTasks?processId="+processId,
	    ajaxLoad : true,
...
function initMethod(){
	$("#tableDiv").on("click",".toLeader",function(){
		...
		ids = ids.join();
		var options = {}
		options.url = Util.getRootPath() + "/w/fileidentify/identifyInfo/toModifyRetentionView?tablename="+tablename+"&ids="+ids+"&flag=HANDLE";
	...
	$("#tableDiv").on("click",".approve",function(){
		...
		//处理流程
		Util.ajaxJsonSync(
			Util.getRootPath() + "/w/fileidentify/identifyInfo/handProcess",
			{	
				handParam: handParam,
			},
			function(result){
